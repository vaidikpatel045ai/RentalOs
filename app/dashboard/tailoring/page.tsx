...
import { TailoringStatusSelect } from "@/components/domain/job-status-select";
import { enumOptions } from "@/lib/format-enum";
import { TAILORING_STATUSES, TAILORING_PRIORITIES } from "@/lib/validations/job";
import type { Priority, TailoringStatus } from "@prisma/client";

export default async function TailoringPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string; priority?: string; branchId?: string; page?: string; pageSize?: string }>;
}) {
  const { status, priority, branchId, ...paginationParams } = await searchParams;
  ...
  const where = {
    ...branchWhere,
    ...(status ? { status: status as TailoringStatus } : {}),
  ...
  const filters: FilterConfig[] = [
    { key: "status", label: "Status", options: enumOptions(TAILORING_STATUSES) },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl">Tailoring</h1>
          <p className="text-sm text-muted-foreground">{totalCount} jobs total</p>
        </div>
        {session?.user.role && can(session.user.role, "tailoring", "create") && (
          <Button asChild>
            <Link href="/dashboard/tailoring/new">
              <Plus className="size-4" /> Assign Job
            </Link>
          </Button>
        )}
      </div>
